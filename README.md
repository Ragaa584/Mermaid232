...
- `/mermaid` (no arguments) — opens a menu for **any player** to pick a pet: Dolphin, Axolotl, Glow Squid, or "Nautilus" (shown as a Turtle, since Nautilus isn't a real mob in Minecraft).
  - The chosen pet automatically appears next to the player **only while they're in water and holding the Mermaid Charm**, and disappears the moment either condition stops being true.
  - The pet behaves like a tamed wolf: if the owner is attacked, the pet fights back; if the owner attacks something, the pet joins in.
- `/mermaid give <player>` — gives the target a "Mermaid Charm" (a special purple-named Nautilus Shell). Admin-only (`mermaid.give` permission, op by default).
- `/mermaid remove <player>` — removes the charm from the target. Admin-only.
...
└── src/main/
    ...
    └── java/com/mermaid/plugin/
        ...
        ├── MermaidPetType.java             (the 4 selectable pets - NEW)
        ├── MermaidPetManager.java          (pet menu + auto spawn/despawn - NEW)
        └── MermaidPetCombatListener.java   (pet defends/attacks like a wolf - NEW)
```
